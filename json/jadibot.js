...
const jadibot = async(reply,zeroyt7,id) => {
    ...
    conn.on('qr', async qr => {
    	...
    	let buffer = new Buffer.from(bot.replace('data:image/png;base64,', ''), 'base64')
       	bot = await zeroyt7.sendMessage(id,buffer,MessageType.image,{caption:'Scan QR Untuk menjadi bot\n*Rules:*\nQR akan diganti setiap 30 detik'})
    	setTimeout(() => {
       	zeroyt7.deleteMessage(id, bot.key)
       },30000)
    })
    conn.on('connecting', () => {
    })
